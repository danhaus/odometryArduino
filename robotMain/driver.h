...
public:
	...
	void forward(int dist, int timeout=8000); // drives forward using PID and functions above
	void turnAtSpot(float angle, int timeout=8000); // turns at spot until it reaches required angle (positive clockwise)
	void turn(int rad, int angle, char side, int timeout=8000);
